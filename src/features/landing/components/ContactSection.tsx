...
const ContactSection = () => {
  ...
  const bgShapeX = useTransform(scrollYProgress, [0, 1], [-30, 30]);
  const formY = useTransform(scrollYProgress, [0.1, 0.5], [60, 0]);

  return (
    <section ref={sectionRef} id="contact" className="py-24 md:py-32 bg-white relative overflow-hidden">
      {/* Background Decor with parallax */}
      <motion.div
        className="absolute top-0 left-0 w-1/3 h-full bg-blue-50/50 z-0 hidden lg:block rounded-r-[100px]"
        style={{ x: bgShapeX }}
      />
      <ContactBackground />
          ...
          <div className="lg:w-1/3 space-y-10">
            ...
            <div className="space-y-4">
              {contactInfo.map((info, index) => (
                <motion.div
                  key={index}
                  className="flex items-start gap-4 p-4 rounded-2xl hover:bg-blue-50/50 transition-all duration-300 cursor-pointer group"
                  initial={{ opacity: 0, x: -40, filter: "blur(4px)" }}
                  whileInView={{ opacity: 1, x: 0, filter: "blur(0px)" }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                  viewport={{ once: true }}
                  whileHover={{ x: 8, backgroundColor: "rgba(239,246,255,0.5)" }}
                >
                  <motion.div
